...
def test_get_person_details(es_data_setup: List[Tuple[str, EntityBase]], redis_data_setup):
    ...
    assert response == get_expected_person(person.to_dict(), films)
...
def test_get_all_persons(es_data_setup: List[Tuple[str, EntityBase]], redis_data_setup):
    persons = [entity.to_dict() for index, entity in es_data_setup if index == ES_PERSONS_INDEX_NAME]
...
def test_sort_persons(es_data_setup: List[Tuple[str, EntityBase]], redis_data_setup):
    persons = [person.to_dict() for index, person in es_data_setup]
...
def test_pagination_persons(es_data_setup: List[Tuple[str, EntityBase]], redis_data_setup):
    persons = [person.to_dict() for index, person in es_data_setup]
    count_pages = 5
